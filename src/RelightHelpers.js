...
/**
 * Parses a list of IIIF services to get the matching servoce type.
 * @param services IIIF service object from Mirador
 * @param {string} serviceType the requested serviceType needed for v2 vs v3 manifests
 * @returns {*} The service object with a profile matching the speficied serviceType
 */
export function getService(services, serviceType) {
  let service;
  services.forEach(function (element) {
    
    console.log(element.profile);
    
    if (element.profile === serviceType) {
      service = element;
    }
  });
  return service;
...
export function getMap(annotationBodies, mapType) {
  ...
  annotationBodies.forEach(function (element) {
    ...
    let service = getService(element.__jsonld.service, 'http://iiif.io/api/annex/services/lightingmap')
    
    // anticipate future edge case now, we can always fix this at a later date
    // the == null is equal to undefined
    if (service == null) {
      service = getService(
          element.__jsonld.service,
          'http://iiif.io/api/extension/lightingmap'
      );
    }

    const services = element.__jsonld.service;

    if (service !== null) {
      if (service.mapType === mapType) {
